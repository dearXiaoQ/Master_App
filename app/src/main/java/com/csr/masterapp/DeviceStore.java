...
public class DeviceStore {
	...
	private LinkedHashMap<Integer, SingleDevice> mSingleDevice = new LinkedHashMap<Integer, SingleDevice>();
	...
	static final String DEVICE_SHORTNAME_KEY = "shortname";
	static final String DEVICE_UUID_KEY = "uuid";
	...
	private String TAG = "DeviceStore";
	 ...
	 * @return
	 * 		  If not search the elements of all devices (id、shortname、version)
	 */
	public ArrayList<DeviceType> getDeviceType() {
		return mDataBase.getAllDeviceType();
	}
	...
	public void addDevicetype(DeviceType type) {
		mDataBase.createOrUpdateType(type);
	}
	...
	private void addSingleDevice(SingleDevice device, boolean storeInDatabase) {
		...
		mSingleDevice.put(device.getDeviceId(), new SingleDevice(device));
	...
	public boolean setConfigurationFromJson(String jsonStr, String networkKey) {
		...
		try {
			...
			for(int devicesIndex=0; devicesIndex < devices.length(); devicesIndex++) {
				...
				String uuid = devices.getJSONObject(devicesIndex).getString(DEVICE_UUID_KEY);
				...
				String shortName = devices.getJSONObject(devicesIndex).getString(DEVICE_SHORTNAME_KEY);
...
//				long modelSupportHigh = devices.getJSONObject(devicesIndex).getLong(DEVICE_MODELSUPPORT_HIGH);
				...
				SingleDevice device = new SingleDevice(deviceId, uuid, uuidHash, deviceName, shortName, 0, 0);
	 ...
	 * Get a all Scenes.
	 * @return
	 * 		  List Scenes if it exists. If not the method will return null.
	 */
	public ArrayList<SceneModel> getScenes() {
		return mDataBase.getAllSecnes();
	}
	...
	public ArrayList<DeviceStream> getDeviceStream() {
		return mDataBase.getDeviceStream();
	}
}
